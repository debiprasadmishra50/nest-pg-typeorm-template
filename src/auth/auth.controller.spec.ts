...
describe("AuthController", () => {
    ...
    describe("resetPassword", () => {
        let result;
        beforeEach(async () => {
            result = await authController.resetPassword(token,{password:mockUser.password,passwordConfirm:mockUser.password});
        });

        it("should be called with authService.forgotPassword", () => {
            expect(authService.resetPassword).toBeCalledWith(token, { password: mockUser.password, passwordConfirm: mockUser.password });
        });

        it("should reset password for user", () => {
            expect(result).toEqual({
               status: "success", user:mockUser, token
            });
        });
    });
